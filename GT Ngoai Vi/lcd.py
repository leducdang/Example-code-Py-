...
BUS_NUMBER = 1 if RPI_REVISION == 1 else 1
...
class Lcd:
    ...
    # put string function
    def lcd_display_string(self, string, line, colwn):
        vitri = 0x00
        if line == 1:
            vitri = 0x80
        if line == 2:
...
try:
    ...
    LCD1= Lcd(0X27)  
    while True:
        LCD1.lcd_display_string("test", 1, 8)
        sleep(1)
        GPIO.output(LED1, 1)
        sleep(1)
        LCD1.lcd_display_string("lcd", 2, 8)
        sleep(1)
        GPIO.output(LED2, 1)
        sleep(1)
        LCD1.lcd_display_string("ok", 3, 8)
        sleep(1)
        LCD1.lcd_clear()
        GPIO.output(LED2, 0)
        GPIO.output(LED1, 0)
